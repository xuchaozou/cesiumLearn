...
vec3 ShadeSurface(const in C_Ray ray, const in C_HitInfo hitInfo, const in C_Surface surface, const in C_Material material)
{
    ...
    vec3 cDiffuseVec3 = vec3(0.0 , 0.0 , 0.0);
    vec3 cSpecularVec =  vec3(0.0 , 0.0 , 0.0);

    shading.cDiffuse = cDiffuseVec3;
    shading.cSpecular = cSpecularVec;
...
void GetCameraRay( const in vec3 vPos, const in vec3 vForwards, const in vec3 vWorldUp, const in vec2 uv, out C_Ray ray)
{
    vec2 vViewCoord = uv;
...
void GetCameraRayLookat( const in vec3 vPos, const in vec3 vInterest, const in vec2 uv, out C_Ray ray)
{
    vec3 vForwards = normalize(vInterest - vPos);
    vec3 vUp = vec3(0.0, 1.0, 0.0);

    GetCameraRay(vPos, vForwards, vUp, uv, ray);
}
